    ...
    let code = match cmd {
        ...
        "resume-process" => {
            let Some(pid) = pid_flag(&args) else {
                eprintln!("kullanim: chimera-admin resume-process --root DIR --share HEX --share HEX --pid <PID>");
                std::process::exit(2);
            };
            cmd_privileged(&root, &args, move |unlock| Request::ResumeProcess { unlock, pid }, "SUREC DEVAM ETTIRME")
        }
        "terminate-process" => {
            let Some(pid) = pid_flag(&args) else {
                eprintln!("kullanim: chimera-admin terminate-process --root DIR --share HEX --share HEX --pid <PID>");
                std::process::exit(2);
            };
            // KALICI ve GERI ALINAMAZ. Operatorun bunu yanlislikla
            // calistirmadigindan emin olmasi icin komut, isini yapmadan
            // ONCE ne yapacagini acikca yazar. (Ek bir "onayliyor musunuz"
            // istemi KASITLI olarak yok: gercek kapi Shamir(2,3) paylari,
            // bir klavye tusu degil.)
            eprintln!("UYARI: pid={pid} KALICI olarak sonlandirilacak. Bu islem GERI ALINAMAZ.");
            eprintln!("       (Geri alinabilir alternatif: 'resume-process' -- sureci devam ettirir.)");
            cmd_privileged(&root, &args, move |unlock| Request::TerminateProcess { unlock, pid }, "SUREC SONLANDIRMA")
        }
        other => {
...
/// `--pid` degerini okur ve GERCEKTEN bir u32 oldugunu dogrular. Gecersiz
/// bir deger sessizce 0'a duserse (ve 0 = System Idle Process ise) cok
/// kotu bir surprize yol acardi; bu yuzden ayristirilamayan bir deger
/// komutu calistirmadan REDDEDILIR.
fn pid_flag(args: &[String]) -> Option<u32> {
    flag(args, "--pid")?.parse::<u32>().ok()
}
