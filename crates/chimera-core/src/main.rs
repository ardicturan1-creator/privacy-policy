...
fn cmd_serve(root: &Path) -> i32 {
    ...
    // --- Siber yaniltma: decoy dosyalar + izleyici + DEVRE KESICI ---
    //
    // Turn 7: bu tuketici artik PASIF DEGILDIR. Her tuzak olayinda
    //   (a) yazan surecin PID'i (Windows'ta Restart Manager ile) alinir,
    //   (b) PID biliniyorsa `circuit_breaker::trip` cagrilir -- surec
    //       ASKIYA ALINIR (sonlandirilmaz) ve insan onayi kuyruguna duser,
    //   (c) PID bilinmiyorsa (bkz. decoy.rs'teki durust sinir) yalnizca
    //       alarm kaydi dusur -- sahte bir "engellendi" iddiasi YOKTUR,
    //   (d) tuzaga hic dokunulmasa bile calisan entropi/hiz heuristigi
    //       ayni devre kesiciyi tetikleyebilir.
    ...
    // Heuristik sayac, decoy tuketici thread'i ile IPC istek thread'leri
    // ARASINDA paylasilir. Sebep somuttur: operator `resume-process` ile
    // "bu yanlis pozitifti" dedigi anda, o PID'in sayaci UNUTULMALIDIR --
    // aksi halde gercekten kotu niyetli olup yanlislikla devam ettirilen
    // bir surec bir daha ASLA alarm uretemezdi.
    let heuristic_state: Arc<Mutex<heuristic::RansomHeuristic>> =
        Arc::new(Mutex::new(heuristic::RansomHeuristic::default()));
    {
        let decoy_alerts = Arc::clone(&decoy_alerts);
        let heuristic_state = Arc::clone(&heuristic_state);
        let l_path = l.deception_log();
        let audit_path = l.audit_log();
        let root_for_cb = l.root.clone();
        std::thread::spawn(move || {
            let audit = |event: &str, detail: &str| {
                let _ = auditlog::append(&audit_path, event, detail);
            };
            for alert in decoy_rx {
                let line = format!(
                    "{{\"ts\":{},\"type\":\"decoy_touch\",\"path\":\"{}\",\"kind\":\"{}\",\"pid\":{},\"app\":\"{}\"}}",
                    alert.ts,
                    alert.path.replace('"', "'"),
                    alert.kind,
                    alert.pid.map(|p| p.to_string()).unwrap_or_else(|| "null".into()),
                    alert.app_name.as_deref().unwrap_or("").replace('"', "'")
                );
                append_line(&l_path, &line);
                decoy_alerts.lock().unwrap().push(line);

                let Some(pid) = alert.pid else {
                    // Yazan surec tespit EDILEMEDI (dosya tanitici zaten
                    // kapanmis olabilir, ya da platform Windows degil).
                    // Bunu "engelledik" gibi yazmak YALAN olurdu.
                    audit(
                        "decoy.touch_unattributed",
                        &format!("{} -- yazan surec tespit edilemedi, devre kesici HEDEFLENEMEDI", alert.path),
                    );
                    continue;
                };

                // 1) Tuzaga dokunma: dogrudan tetikler.
                // Ayni surec ONCEDEN yakalandiysa yeniden tetikleme: bir
                // fidye yazilimi tuzak dizinindeki ONLARCA dosyaya birden
                // dokunur ve her biri icin yeni bir "askiya al + blokla"
                // turu calistirmak denetim kaydini gurultuye bogardi
                // (surec zaten donmus durumda ve kuyrukta bekliyor).
                if heuristic_state.lock().unwrap().has_tripped(pid) {
                    audit("decoy.touch_already_tripped", &format!("pid={pid} zaten insan onayi kuyrugunda: {}", alert.path));
                    continue;
                }

                let outcome = circuit_breaker::trip(
                    &root_for_cb,
                    pid,
                    &circuit_breaker::TripReason::DecoyTouched { path: alert.path.clone() },
                    &audit,
                );
                if !outcome.skipped_protected {
                    // Bu PID artik kuyrukta: yukaridaki `has_tripped`
                    // kontrolu bundan sonra tekrar tetiklenmesini onler.
                    heuristic_state.lock().unwrap().mark_tripped(pid);
                    continue;
                }

                // 2) Korumali bir surecti (kendi tuzak olusturmamiz gibi):
                //    yine de entropi/hiz sayaci ISLETILIR, cunku bu sayac
                //    tuzaga hic dokunmayan surecler icin de gecerlidir.
                if let Some(h) = heuristic::sample_file_entropy(std::path::Path::new(&alert.path)) {
                    let observed = heuristic_state.lock().unwrap().observe(pid, &alert.path, h, alert.ts);
                    if let Some(trip) = observed {
                        circuit_breaker::trip(
                            &root_for_cb,
                            pid,
                            &circuit_breaker::TripReason::MassEncryption { detail: trip.as_detail() },
                            &audit,
                        );
                    }
                }
...
fn handle_request(
    ...
    match req {
        ...
        Request::ScanNow { unlock } => {
            ...
            let report = pipeline::run_once(root, false, &audit);
        ...
        Request::ListSuspended { unlock } => {
            if !unlocked(&unlock) {
                let _ = auditlog::append(audit_path, "privileged_denied", "ListSuspended");
                return Response::Denied;
            }
            Response::CircuitBreakerOk(circuit_breaker::list_suspended_text(root))
        }
        Request::ResumeProcess { unlock, pid } => {
            if !unlocked(&unlock) {
                let _ = auditlog::append(audit_path, "privileged_denied", "ResumeProcess");
                return Response::Denied;
            }
            let audit_owned = audit_path.to_path_buf();
            let audit = move |e: &str, d: &str| { let _ = auditlog::append(&audit_owned, e, d); };
            match circuit_breaker::resume(root, pid, &audit) {
                Ok(msg) => {
                    // "Yanlis pozitifti" karari verildi: heuristik sayac da
                    // sifirlanir, boylece bu surec ILERIDE gercekten kotu
                    // davranirsa yeniden yakalanabilir.
                    heuristic_state.lock().unwrap().forget(pid);
                    Response::CircuitBreakerOk(msg)
                }
                Err(e) => Response::CircuitBreakerOk(format!("HATA: {e}")),
            }
        }
        Request::TerminateProcess { unlock, pid } => {
            // KALICI, geri alinamaz aksiyon: yalnizca Shamir(2,3) kapisi
            // acikken calisir ve boru hattinin whitelist'inden ASLA
            // gecmez (bkz. pipeline::is_whitelisted).
            if !unlocked(&unlock) {
                let _ = auditlog::append(audit_path, "privileged_denied", "TerminateProcess");
                return Response::Denied;
            }
            let audit_owned = audit_path.to_path_buf();
            let audit = move |e: &str, d: &str| { let _ = auditlog::append(&audit_owned, e, d); };
            match circuit_breaker::terminate(root, pid, &audit) {
                Ok(msg) => Response::CircuitBreakerOk(msg),
                Err(e) => Response::CircuitBreakerOk(format!("HATA: {e}")),
            }
        }
    }
